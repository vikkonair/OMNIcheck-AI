...
目前 **M10.3.1** 已建立 AI-optional Section Workflow：確定性文字、AI 草稿、工程師審查與核准版本彼此分離；Ollama 尚未啟用，既有 Renderer 不讀 AI 草稿。M10.2 已將同仁 `0.13.2.dev2` 的新版介面透過 Adapter 接上我們的 M10.3.1 後端，並部署公司候選 release `0a6dccd`；登入、Knowledge/CVE 與 GPDB 暫不啟用，`/classic` 保留原介面。
